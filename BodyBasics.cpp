...
///#define SERVER "128.61.125.63"  //ip address of udp server
...
void CBodyBasics::DrawBody(const Joint* pJoints, const D2D1_POINT_2F* pJointPoints)
{
	...
	double SpineMidX = pJoints[JointType_SpineMid].Position.X;
	double SpineMidY = pJoints[JointType_SpineMid].Position.Y;
	
	double SpineShoulderX = pJoints[JointType_SpineShoulder].Position.X;
	double SpineShoulderY = pJoints[JointType_SpineShoulder].Position.Y;

	double ElbowRightX = pJoints[JointType_ElbowRight].Position.X;
	...
	double armX = rightShoulderX - ElbowRightX;
